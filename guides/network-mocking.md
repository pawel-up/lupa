...
- `query`: A parsed JavaScript object (`Record<string, string>`) representing the URL query parameters.
- `headers`: A parsed JavaScript object (`Record<string, string>`) of all request headers.

## Request Matching

Lupa gives you flexible ways to define which network requests your mock should intercept. You can pass a plain string (with glob support), a Regular Expression, or an explicit configuration object.
...
### Regular Expressions
For highly dynamic endpoints, you can pass a standard JavaScript RegExp:

```ts
// Matches /api/users/123, /api/users/999
await network.mock(/\/api\/users\/[0-9]+/, { status: 200 })
```
